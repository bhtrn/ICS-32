        ...
        if len(commands) == 0:
            ...
            if commands[0] == 'Q':
                    ...
                    if p2.exists():
                        ...
                        if file_exist == False:
                            ...
                            if len(username) == 0 or len(password) == 0 or len(dsuserver) == 0:
                                ...
                                nProfile1 = NaClProfile.NaClProfile()
                                nProfile1.generate_keypair()
                                nProfile1.dsuserver = dsuserver
                                nProfile1.username = username
                                nProfile1.password = password
                                with open(p2 / new_file, 'w') as p:
                                    p.close()
                                nProfile1.save_profile(str(p2 / new_file))
                            for item in p2.iterdir():
                                if item.name == new_file:
                ...
                if len(commands) < 2:
                    ...
                    p5 = Path(commands[1])
                    if p5.exists():
                        nProfile = NaClProfile.NaClProfile()
                        nProfile.load_profile(str(p5))
                        print("Profile Loaded!")
                        while True:
                            option = input("Would you like to add a post or change your bio?: \n1 = Add Post\n2 = Change Bio\n3 = Delete Post\n4 = Done\nQ = Quit Program\n")
                            if option == '1':
                                post_entry = input("Write Post:\n*Note: Program takes the follwoing keywords: @weather, @lastfm\n")
                                if "@lastfm" in post_entry and "@weather" in post_entry:
                                    apikey = "3a6e68bc4d77a79a78deceb14896de42"
                                    apikey1 = "7329210851dbcf8f44e8ea9a53332888"
                                    API_ZIP = input("Enter Zipcode of desired destination; Ex: '12345'\n")
                                    API_CC = input("Enter Country Code of Desired Destination; Ex: 'US'\n")
                                    API_ART = input("Enter Favorite Artist for top Track\n")
                                    try:
                                        data1 = OpenWeather(API_ZIP, API_CC, apikey)
                                        data2 = LastFM(API_ART, apikey1)
                                        npost = data1.transclude(post_entry)
                                        fpost = data2.transclude(npost)
                                        post = Profile.Post(fpost)
                                        nProfile.add_post(post)
                                        nProfile.save_profile(str(p5))
                                        decrypted_posts = nProfile.get_posts()
                                        for i in decrypted_posts:
                                            if i.entry == fpost:
                                                Dpost = i.entry
                                        send(server=nProfile.dsuserver, port='2021', username=nProfile.username, password=nProfile.password, message=Dpost, keypair=nProfile.keypair)
                                    except:
                                        print("Error Occurred When trying to send post. Please Try Again.")
                                elif "@weather" in post_entry:
                                    apikey = "3a6e68bc4d77a79a78deceb14896de42"
                                    API_ZIP = input("Enter Zipcode of desired destination; Ex: '12345'\n")
                                    API_CC = input("Enter Country Code of Desired Destination; Ex: 'US'\n")
                                    try: 
                                        data = OpenWeather(API_ZIP, API_CC, apikey)
                                        npost = data.transclude(post_entry)
                                        post = Profile.Post(npost)
                                        nProfile.add_post(post)
                                        nProfile.save_profile(str(p5))
                                        decrypted_posts = nProfile.get_posts()
                                        for i in decrypted_posts:
                                        ...
                                        print("Error Occurred When trying to send post. Please Try Again.")
                                elif "@lastfm" in post_entry:
                                    apikey = "7329210851dbcf8f44e8ea9a53332888"
                                    API_ART = input("Enter Favorite Artist for top Track\n")
                                    try:
                                        data = LastFM(API_ART, apikey)
                                        npost = data.transclude(post_entry)
                                        post = Profile.Post(npost)
                                        nProfile.add_post(post)
                                        nProfile.save_profile(str(p5))
                                    ...
                                    decrypted_posts = nProfile.get_posts()
                                    for i in decrypted_posts:
                                        if i.entry == post_entry:
                                            Dpost = i.entry
                                    send(server=nProfile.dsuserver, port='2021', username=nProfile.username, password=nProfile.password, message=Dpost, keypair=nProfile.keypair)
                            elif option == '2':
                                bio_1 = input('Write a bio:')
                                if "@weather" in bio_1 and "@lastfm" in bio_1:
                                    apikey1 = "3a6e68bc4d77a79a78deceb14896de42"
                                    apikey2 = "7329210851dbcf8f44e8ea9a53332888"
                                    API_ZIP = input("Enter Zipcode of desired destination; Ex: '12345'\n")
                                    API_CC = input("Enter Country Code of Desired Destination; Ex: 'US'\n")
                                    API_ART = input("Enter Favorite Artist for top Track\n")
                                    try:
                                        data1 = OpenWeather(API_ZIP, API_CC, apikey1)
                                        data2 = LastFM(API_ART, apikey2)
                                        nbio = data1.transclude(bio_1)
                                        fbio = data2.transclude(nbio)
                                        nProfile.bio = fbio
                                        nProfile.save_profile(str(p5))
                                        send(server=nProfile.dsuserver, port='2021', username=nProfile.username, password=nProfile.password, message=None, keypair=nProfile.keypair, bio=nbio)
                                    except:
                                        print("Error Occurred When trying to change bio. Please Try Again.")
                                        
                                elif "@weather" in bio_1:
                                    apikey = "3a6e68bc4d77a79a78deceb14896de42"
                                    API_ZIP = input("Enter Zipcode of desired destination; Ex: '12345'\n")
                                    API_CC = input("Enter Country Code of Desired Destination; Ex: 'US'\n")
                                    try:
                                        data = OpenWeather(API_ZIP, API_CC, apikey)
                                        nbio = data.transclude(bio_1)
                                        nProfile.bio = nbio
                                        nProfile.save_profile(str(p5))
                                        send(server=nProfile.dsuserver, port='2021', username=nProfile.username, password=nProfile.password, message=None, keypair=nProfile.keypair, bio=nbio)
                                    ...
                                    nProfile.bio = bio_1
                                    nProfile.save_profile(str(p5))
                                    send(server=nProfile.dsuserver, port='2021', username=nProfile.username, password=nProfile.password, message=None,  keypair=nProfile.keypair, bio=bio_1)
                            elif option == '3':
