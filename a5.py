...
import ds_client, ds_protocol, NaClDSEncoder
...
class Body(tk.Frame):
    ...
    def node_select(self, event):
        ...
        entries = self._current_profile.get_posts()
...
class Footer(tk.Frame):
    ...
    def save_click(self):
        try:
            ...
            print("No Profile Loaded or Opened. Please Try Again!")
...
class MainApp(tk.Frame):
    ...
    def new_profile(self):
        try:
            ...
            self._current_profile.username = "Big Brain Man"
            self._current_profile.password = "12345"
            self.body.reset_ui()
            self.body._current_profile = self._current_profile
        except:
            print("No Profile Name! Please Enter a Profile name")
    """
    ...
    def open_profile(self):
        try:
            filename = tk.filedialog.askopenfile(filetypes=[('Distributed Social Profile', '*.dsu')], defaultextension=[('Distributed Social Profile', '.dsu')])
            self._profile_filename = filename.name
            self._current_profile = NaClProfile()
            ...
            print("Invalid File or No File Accessed, Try Again!")
    ...
    def save_profile(self):
        ...
        if self._is_online is True:
            decrypted_posts = self._current_profile.get_posts()
    ...
    def publish(self, post:Post):
        ds_client.send(server=self._current_profile.dsuserver, port='2021', username=self._current_profile.username, password=self._current_profile.password, message=post, keypair=self._current_profile.keypair)
...
if __name__ == "__main__":
    ...
    main.title("ICS 32 Distributed Social Application!")
